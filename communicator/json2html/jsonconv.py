...
class Json2Html:
    ...
    def convert(self, json="", table_attributes='border="1"', clubbing=True, encode=False, escape=True):
        ...
        if not json:
            ...
            json_input = json
        converted = self.convert_json_node(json_input, False)
        if encode:
    ...
    def convert_json_node(self, json_input, asInput):
        """
        ...
        if type(json_input) in text_types:
            if self.escape:
                if (asInput):
                    self.prevId += 1
                    return '<input type="text" id="' + str(self.prevId) + '" name="' + str(self.prevId) + '" value="' + html_escape(text(json_input)) + '"/>'
                else:
    ...
    def convert_list(self, list_input):
        ...
        if column_headers is not None:
            ...
            for list_entry in list_input:
                ...
                self.prevId += 1
                converted_output += '</td><td><input type="text" id="' + str(self.prevId) + '" name="' + str(self.prevId) + '" value="'.join([self.convert_json_node(list_entry[column_header], False) for column_header in
                                                     column_headers])
        ...
        converted_output += '</li><li>'.join([self.convert_json_node(child, True) for child in list_input])
    ...
    def convert_object(self, json_input):
        ...
        converted_output += "</tr><tr>".join([
            "<th>%s</th><td><td>%s</td></td>" %(
                self.convert_json_node(k, False),
                self.convert_json_node(v, True)
            )
